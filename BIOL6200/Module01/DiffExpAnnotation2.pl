...
sub readBlast {
	while (<BLAST>) {
		...
		# Check if transcrip ID exists
		if ($transcriptToProtein{$transcriptId}){ 
			# say "ID $transcriptToProtein{$transcriptId} exists.";
			if($pident > 99){
				$transcriptToProtein{$transcriptId} = $swissProtBase; #Hash storing BLAST mappings pident > 99
				# say "pident = $pident, ID stored";
			}
		}else {
			$transcriptToProtein{$transcriptId} = $swissProtBase; #Hash storing BLAST mappings
		}
...
sub readGeneToGO {
	while (<GENE_TO_GO>) {
		...
		if ( defined $DB_Object_ID && defined $GO_ID ) {
			...
			if ( not defined $geneToGo{$DB_Object_ID}{$GO_ID} ) {
				$geneToGo{$DB_Object_ID}{$GO_ID} = 1;
			}
		}
	}
}
	...
	while (<DIFF_EXP>) {
		...
		if ( defined $transcriptToProtein{$Sp_ds} ) {
			...
			if ( defined $geneToGo{$proteinId} ) {
				my $counter = 0;
				foreach my $GO_ID (sort keys $geneToGo{$proteinId}){
